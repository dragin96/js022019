    ...
    function textNodeRecursive(where) {
        ...
        while (i < childrens.length) {
            if (childrens[i].nodeType === 3) {
                ...
                text.tags[tagName] = (text.tags.hasOwnProperty(tagName)) ? text.tags[tagName] + 1 : 1;

                if (classes) {
                    for (let i = 0; i < classes.length; i++) {
                        text.classes[classes[i]] = (text.classes.hasOwnProperty(classes[i]))
                            ? text.classes[classes[i]] + 1 : 1;
                    }
                }
            }
